...
- **Allowances**: Konnect credit balance `GET .../users/{externalUserId}/allowances` (`live` spendable / `settled` ledger / `pending` unbooked grants). Plan included remaining is `GET .../usage/balance?externalUserId=...`. Free grants are admin-only (`POST /api/v1/admin/billing/owners/{userId}/grants` via customer-service).
...
2. Browser: `verification_uri_complete` (when third-party device login is enabled) points **directly** at the integrator `initiate_login_uri` with `iss` + `target_link_uri` (must be OP origin + path `/oidc/device` with query params). Hand-typed `verification_uri` (`/oidc/device`) still uses `/oidc/device/initiate-login` (skip cookie) when opted in. After approval, a return to `target_link_uri` is safe because `/oidc/device` gates on a bound DeviceCode (`accountId`/`grantId`) and shows a terminal success page instead of re-federating.
