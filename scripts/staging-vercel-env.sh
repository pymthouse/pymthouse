...
# App / M2M Starter seed (USD micros). Owner Starter is platform_billing_settings.
STARTER_INCLUDED_USD_MICROS="${OPENMETER_DEFAULT_STARTER_INCLUDED_USD_MICROS:-0}"
...
if [[ -n "${OPENMETER_INGEST_API_KEY:-}" ]]; then
  add_env OPENMETER_INGEST_API_KEY "$OPENMETER_INGEST_API_KEY"
fi
