...
*By Ryan Lopopolo, Member of the Technical Staff — February 11, 2026 — Engineering*
...
![Diagram: Codex drives the app with Chrome DevTools MCP to validate its work — Codex selects a target and clears the console, snapshots the app before, triggers a UI path, captures runtime events during the interaction, snapshots after, applies a fix and restarts, then loops re-running validation until clean.](https://images.ctfassets.net/kftzwdyauwt9/4Xr18TZ5G4Bh8zIgsTFIVK/f7ae689ddd8c31664e39d809b0973425/OAI_Harness_engineering_Giving_Codex_a_full_observability_stack_desktop-light__1_.svg)
...
![Diagram: Giving Codex a full observability stack in local dev — the app sends logs (HTTP), OTLP metrics, and OTLP traces to Vector, which fans out (locally) to an observability-stack-services cluster containing Victoria Logs, Victoria Metrics, and Victoria Traces, each fronted by LogQL, PromQL, and TraceQL APIs. Codex uses these signals to query, correlate, and reason; then implements a change (PR), restarts the app, re-runs the workload from the codebase, tests UI journeys, and repeats in a feedback loop.](https://images.ctfassets.net/kftzwdyauwt9/4Xr18TZ5G4Bh8zIgsTFIVK/f7ae689ddd8c31664e39d809b0973425/OAI_Harness_engineering_Giving_Codex_a_full_observability_stack_desktop-light__1_.svg)
...
![Diagram: The limits of agent knowledge — What Codex can't see doesn't exist. Codex's knowledge sits in a bounded bubble; below it, unseen sources (a Google Doc on feature prioritization, a Slack message setting security-posture guidance, and tacit knowledge such as "Ryan owns architectural direction") are invisible. The only path to make them visible is to encode them into the codebase as markdown.](https://images.ctfassets.net/kftzwdyauwt9/7uWHsJIC6o3uQPsnQ2Avz9/8be3e321892054bd215afb2b250a176a/OAI_Harness_engineering_The_limits_of_agent_knowledge_desktop-light.png)
