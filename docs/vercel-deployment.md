...
| **Staging** | `https://staging.pymthouse.com` | manual dispatch on [deploy-staging-vercel.yml](../.github/workflows/deploy-staging-vercel.yml) (choose any branch) when `VERCEL_PREVIEW_AUTO_DEPLOY=true` |

Staging works by force-pushing the chosen branch onto a literal `staging` branch; Vercel's Git integration builds and aliases `staging.pymthouse.com` from that push (the domain is bound to the `staging` branch under Preview in Project Settings -> Domains — Vercel domains can't be aliased ad hoc via CLI once connected to an environment). The project's Ignored Build Step is a conditional script (not a blanket `exit 1`) that only allows a build when `VERCEL_GIT_COMMIT_REF` is `staging`; pushes to `main` or any other branch are still ignored, so CI owns production.
...
1. Connect the same GitHub repo (required — staging now relies on Vercel's native Git integration to build and alias pushes to the `staging` branch).
2. Set **Ignored Build Step** to the conditional command above so only the `staging` branch auto-builds (CI owns production via tags).
3. Bind `staging.pymthouse.com` to the `staging` branch under Preview in Project Settings -> Domains.
4. Configure env vars under **Production** and **Preview** scopes — see [.env.vercel.template](../.env.vercel.template).
...
| `VERCEL_PREVIEW_AUTO_DEPLOY=true` | [deploy-staging-vercel.yml](../.github/workflows/deploy-staging-vercel.yml), manual dispatch with a `branch` input | Vercel's Git integration deploys staging from the `staging` branch |
...
**Avoid double deploys:** In the **pymthouse** Vercel project → Settings → Build and Deployment → **Ignored Build Step**, set a conditional command so native Git integration only builds pushes to the `staging` branch (CI owns production):
...
```bash
# GitHub Actions → Deploy staging (Vercel) → Run workflow → enter the branch to deploy
```
