...
### Two Vercel projects (production vs staging)
...
2. Set **Ignored Build Step** to `exit 1` so Git pushes do not auto-deploy (CI/manual deploy owns staging).
3. Configure env vars under **Production** only (not Preview) — see [.env.vercel.template](../.env.vercel.template).
...
**GitHub secret required for CI:** `VERCEL_TOKEN` (same token used for CLI deploys).
