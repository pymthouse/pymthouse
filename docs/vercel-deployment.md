...
- Set `DATABASE_URL` on the Vercel project (Production scope) for **runtime**. Production CI also requires the same URL as the `DATABASE_URL` secret on the GitHub `vercel / production` environment — [deploy-production-vercel.yml](../.github/workflows/deploy-production-vercel.yml) runs `npm run db:prepare` **before** `vercel build` so schema migrations land before traffic hits new code. Local/Vercel-cloud `prebuild` still skips migrate when `VERCEL=1` (preview builds must not touch production Neon).
