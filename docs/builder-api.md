...
**Wallet boundary (UI):** Owner **`/billing`** shows only the shared **owner wallet** (prepaid strip + Owner Starter progress) — end-user Starter spend does **not** roll into that bar. The Usage dashboard (`/usage`, `/apps/{id}/usage`, powered by `GET /api/v1/billing/dashboard`) aggregates tenant meter usage with a per-identity table that includes end-user **spendable** and **plan remaining** (same mint-gate inputs). App cards split **End-user usage** vs **Your usage**. App owners/admins can open **All Users** at `/apps/{id}/usage/users` (provisioned `app_users` + cycle spend/spendable/sub) and customer detail at `/apps/{id}/users/{externalUserId}` via session APIs `GET /api/v1/apps/{clientId}/customers` and `…/customers/{externalUserId}`. Builder M2M balance for one user remains `GET /api/v1/apps/{clientId}/usage/balance?externalUserId=...`.
...
2. **No catalog/pricing fetch on signing:** direct DMZ signing does not load dashboard pricing for validation. **`GET /api/v1/pipeline-pricing`** remains for UIs but discovery-service raw has no pricing rows (empty list).
