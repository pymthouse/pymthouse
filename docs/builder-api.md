...
- **`verification_uri_complete`** — Prefer this for browser open. When third-party device login is enabled, it points **directly** at your registered **`initiate_login_uri`** with OIDC Core third-party login params (`iss`, `target_link_uri`, optional `login_hint`). `target_link_uri` embeds `user_code`, `client_id`, and `iss` and must stay on the OP origin at path `/oidc/device`. When third-party login is off (or the initiate URI is invalid), complete is the device page with those query params.

Unauthenticated users who open the plain **`verification_uri`** (or typed device page) may still be redirected once through `/oidc/device/initiate-login` when the app opts in (that hop sets a short-lived skip cookie for pre-approval returns). The redirect target is loaded **from the database for `client_id`** (open-redirect safe). Federated apps should complete approval via RFC 8693 on the RP (not the OP device form with a local PymtHouse session).

**Opt-in:** Enable **Redirect device verification to initiate login URI** and set **Initiate login URI** to your HTTPS endpoint that accepts `iss`, `target_link_uri`, and optional `login_hint`. Validate `iss` against discovery and validate `target_link_uri`. **Option B (NaaP):** after login, mint a user JWT via Builder, then call `POST {issuer}/token` with token exchange and `resource=urn:pmth:device_code:<user_code>` (M2M Basic auth), and show a success screen on the RP instead of sending the browser back to `target_link_uri`. If you do return to `target_link_uri` after approval, `/oidc/device` detects the bound DeviceCode and shows a terminal "Device approved" page (no re-federation).
...
| --- | --- | --- | --- |
| `GET` | `/api/v1/apps/{clientId}/billing/stripe` | Provider session | Merchant Connect status (`stripeConnectedAccountId`, charges/payouts flags, `applicationFeeBps`, `billingMode`, `endUserCap`, `activation`) + OM profile ids. `connectPlanes.live` / `connectPlanes.sandbox` report each plane's parked onboarding (`stripeConnectedAccountId`, `chargesEnabled`, `detailsSubmitted`, `connectedAt`, `ready`) or `null` when that plane was never set up — the active one is whichever `stripeLivemode` selects. |
| `POST` | `/api/v1/apps/{clientId}/billing/stripe/connect` | App **owner** or platform admin | Start merchant Connect via Account Links: `{ mode?: "account_link", stripeLivemode?: boolean }` (default mode `account_link`). `stripeLivemode` is the Payments Live/Sandbox toggle; persist it before creating the Connected Account so Live onboarding does not fall through to the sandbox default. Reuses the plane's parked Connected Account when one exists, otherwise creates an Express/Accounts v2 account; returns Stripe-hosted `{ url }`. `mode: "oauth"` is rejected. |
...
| `PATCH` | `/api/v1/apps/{clientId}/billing/stripe` | App owner/admin | Update `progressiveBilling`, `softNegativeUsdMicros`, `invoiceLeadUsdMicros`, `applicationFeeBps`, `billingMode`, `stripeLivemode`, and/or `endUserCap`. Do not send `billingMode` and `stripeLivemode` in the same body (400) — merchant readiness is judged against the active plane, so switch first, then set mode. `stripeLivemode` moves the app between the sandbox and live Stripe planes: the plane being left is parked (its Connected Account, capability flags, and `connectedAt` survive) and the target plane's parked row is restored and re-synced from Stripe, so sandbox → live → sandbox needs no re-onboarding. The response carries `stripeLivemodeSwitch` (`changed`, `livemode`, `accountId`, `ready`); `accountId: null` means that plane still needs onboarding, and an app left in `billingMode: "merchant"` on an unready plane cannot sell paid plans until it finishes. `softNegativeUsdMicros` is the app-wide overage limit — the unbilled debt a subject may accrue while spendable is $0 (mint/signer deny at/above). It must be `0` (no limit) or at least `2000000` ($2.00); smaller limits deadlock, because every invoice raised on the way there falls under Stripe's $0.50 minimum charge. Unset (null) reads as the `2000000` ($2.00) default, so an app never grants unbounded debt unless the owner explicitly sets `0`. `invoiceLeadUsdMicros` overrides the debt level that triggers an invoice, which otherwise derives as `min(ceiling / 2, $5)`. Switching to `merchant` requires Connect ready (`charges_enabled` + `details_submitted`). Mode switches are **mint-forward** only (identity / provision / customer caches ≈ 300s + live signer JWT TTL); response includes `billingModeEffectiveAt`. Already-ingested usage stays on the customer it was billed to. |
...
| `DELETE` | `/api/v1/apps/{clientId}/billing/stripe` | App **owner** or platform admin | Disconnect merchant Connect (+ clear OM Stripe profile ids). Drops only the **active** plane's onboarding — the other plane stays parked and is restored by a `stripeLivemode` switch. Use the switch, not disconnect, to move between sandbox and live. |
...
`GET /api/v1/apps/{clientId}/billing/state` answers the three questions an integrator has about a subject: can it spend right now, how much room is left, and what happens next. Merchant apps must pass `externalUserId`; owner-rollup apps may pass one to scope unbilled debt to a single subject. The same object is embedded as `billingState` in `GET …/billing/wallet` and returned by `POST …/billing/collect`, so a read and a rejection never disagree.

Every amount is a `Money` object — `{ usdMicros, usd, currency }` — because `app_billing_config.default_currency` drives settlement and a bare `usdMicros` on the wire loses that.

```json
{
  "asOf": "2026-08-08T03:14:00.000Z",
  "subject": { "type": "end_user", "externalUserId": "user-123", "billingMode": "merchant" },
  "status": "at_risk",
  "canSpend": true,
  "reason": null,
  "funding": {
    ...
    "overage": {
      ...
      "unbilledDebt": { "usdMicros": "6000000", "usd": "6.00", "currency": "USD" },
      "remaining": { "usdMicros": "4000000", "usd": "4.00", "currency": "USD" },
      "utilizationBps": 6000,
  ...
  "collection": {
    ...
    "leadThreshold": { "usdMicros": "5000000", "usd": "5.00", "currency": "USD" },
    "minimumCharge": { "usdMicros": "500000", "usd": "0.50", "currency": "USD" },
    "cycle": "MONTH",
    "collectionInterval": "DAY",
    "lastRaisedAt": "2026-08-08T02:59:00.000Z",
    "nextRaiseEligibleAt": "2026-08-08T03:04:00.000Z"
  },
  "explain": {
    "headline": "Collecting recent usage",
    "detail": "An invoice for your recent usage is being collected. $4.00 of your $10.00 spending buffer is left; requests keep working unless the buffer runs out first.",
    "docsUrl": "https://docs.pymthouse.com/billing/pay-as-you-go"
  }
}
```
...
| --- | --- | --- |
| `active` | `true` | Prepaid credits or included plan usage remain. Usage draws those down first. |
| `overage` | `true` | Credits are gone. Usage is invoiced automatically as it accrues. |
...
`reason` is set only when `status` is `blocked`. The same codes appear as an additive `reason` field on the OAuth `402` from the signer-token mint (alongside the existing `error` code, which keeps its OAuth meaning), so a client can branch on one vocabulary.

| `reason` | `nextAction` | Cause |
| --- | --- | --- |
| `no_payment_method` | `add_payment_method` | Credits exhausted and no default card on file, so usage cannot be billed. |
| `overage_not_available` | `add_funds` | Credits exhausted and this account is not overage-eligible (for example the sandbox Starter's hard gate). |
| `debt_ceiling_reached` | `awaiting_settlement` | Unbilled debt hit the overage limit while payment is still being collected. Clears on its own once the invoice settles. |
| `billing_unavailable` | `none` | Allowance could not be confirmed upstream. Usually transient. |
