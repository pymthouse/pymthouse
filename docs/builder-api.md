...
| `POST` | `/api/v1/apps/{clientId}/billing/stripe/connect` | App **owner** or platform admin | Start merchant Connect via Account Links: `{ mode?: "account_link", stripeLivemode?: boolean }` (default mode `account_link`). `stripeLivemode` is the Payments Live/Sandbox toggle; persist it before creating the Connected Account so Live onboarding does not fall through to the sandbox default. Creates Express/Accounts v2 Connected Account if needed, returns Stripe-hosted `{ url }`. `mode: "oauth"` is rejected. |
