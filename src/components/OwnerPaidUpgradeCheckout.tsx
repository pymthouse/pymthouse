...
// ---------------------------------------------------------------------------
// Error taxonomy
// Server error codes → user-facing copy + recovery action.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Idempotency key — rotates when plan selection changes
// ---------------------------------------------------------------------------

function makeIdempotencyKey(planKey: string): string {
  const ts = Date.now();
  const rand = Math.random().toString(36).slice(2, 8);
  return `owner-upgrade:${planKey}:${ts}:${rand}`;
}
  ...
  if (t.includes("visa"))
    return (
      <svg viewBox="0 0 38 24" className="h-4 w-6 shrink-0" aria-hidden="true">
        ...
        <path d="M16 7l-3 10h-2L8 9.5c-.2-.6-.4-.8-.8-1A6.5 6.5 0 005 8l.1-.5h3.2c.4 0 .8.3.9.7l.8 4.3 2.1-5zm1.8 0h2l-1.5 10h-2zm8 6.5c0-1.7-2.4-1.8-2.4-2.6 0-.2.2-.5.8-.6.5 0 1 .1 1.4.3l.3-1.3A4 4 0 0024.5 9c-2.1 0-3.5 1.1-3.5 2.7 0 1.2 1 1.8 1.8 2.2.8.4 1 .7 1 1s-.4 1-1.2 1c-.7 0-1.4-.2-2-.5l-.3 1.3c.6.3 1.3.4 2 .4 2.2 0 3.5-1 3.5-2.6zm5-6.5l-3 10h-2l3-10h2z" fill="white" />
      </svg>
    );
  if (t.includes("mastercard"))
    return (
      <svg viewBox="0 0 38 24" className="h-4 w-6 shrink-0" aria-hidden="true">
        ...
        <path d="M19 7.3A7 7 0 0122.6 12 7 7 0 0119 16.7 7 7 0 0115.4 12 7 7 0 0119 7.3z" fill="#FF5F00" />
      </svg>
    );
  // Generic card icon
  return (
    <svg viewBox="0 0 24 24" className="h-4 w-5 shrink-0 text-zinc-400" fill="none" aria-hidden="true">
...
function ErrorBanner({ code, raw }: { code: ErrorCode; raw?: string }) {
  ...
  return (
    ...
    >
      <svg viewBox="0 0 20 20" className="mt-0.5 h-4 w-4 shrink-0 text-red-400" fill="currentColor" aria-hidden="true">
...
const PLAN_DETAIL: Record<string, { headline: string; bullets: string[] }> = {
  // Keyed by planKey prefix — fallback to generic below
...
function TierCard({ tier, selected, onSelect, disabled, inputName }: TierCardProps) {
    ...
    <label
      className={[
        ...
        selected
          ...
          : "border-white/[0.07] bg-white/2.5 hover:border-white/15",
        ...
        <div className="flex-1 min-w-0">
          ...
          {detail ? (
            <ul className="mt-2 space-y-0.5">
              {detail.bullets.map((b) => (
                <li key={b} className="flex items-start gap-1.5 text-xs text-zinc-400">
                  <span className="mt-0.5 text-emerald-500/70" aria-hidden="true">·</span>
                  {b}
                </li>
              ))}
            </ul>
            ...
            <p className="mt-1.5 text-xs text-zinc-400">
              {formatUsdMicrosSummary(tier.includedUsdMicros)} included usage each billing cycle.
              Overage billed to your card.
            </p>
          )}
        </div>
    ...
    <div className="rounded-xl border border-white/6 bg-white/2.5 p-4">
      ...
      <div className="mt-3 space-y-2">
        ...
        <div className="flex items-baseline justify-between gap-2 text-xs text-zinc-500">
          ...
          <span>{formatUsdMicrosSummary(selected.includedUsdMicros)} value</span>
        </div>
        <div className="flex items-baseline justify-between gap-2 text-xs text-zinc-500">
          <span>Overage</span>
      ...
      <div className="flex items-baseline justify-between gap-2">
        ...
        <span className="font-semibold text-zinc-100">${selected.monthlyFeeUsd}</span>
      </div>
...
export default function OwnerPaidUpgradeCheckout({
  ...
  // Rotate idempotency key when plan changes; never rotate during in-flight.
  const idempotencyKeyRef = useRef<string>(
    makeIdempotencyKey(initialPlanKey ?? ""),
  );
  const lastKeyedPlanRef = useRef<string>(initialPlanKey ?? "");
  ...
  // Prevent body scroll on in-flight state
  useEffect(() => {
    if (busy) {
  ...
  // Prevent accidental navigation while upgrading
  useEffect(() => {
    if (!busy) return;
  ...
  const loadTiers = useCallback(async () => {
    ...
    try {
      ...
      setSelectedKey((prev) => {
        ...
        // Rotate key if first plan differs from initial
  ...
  // Clean up one-shot pm flag from URL without losing plan param
  useEffect(() => {
  ...
  // Guard against double-submit: once setBusy(true), this function bails early
  // on a second invocation. The idempotency key guarantees the server-side is
  // also idempotent for the same plan selection.
  async function confirmUpgrade() {
    ...
    try {
      ...
      if (!res.ok) {
        const msg = body.error || "Upgrade failed";
        setErrorCode(classifyError(msg));
        setErrorRaw(msg);
        setBusy(false);
        return;
      }
      // Success — navigate; keep busy=true so button stays disabled during redirect
  ...
  // Keyboard guard on Enter in the confirm region — prevent accidental submit
  ...
  return (
    <DashboardLayout>
      ...
      {errorCode && <ErrorBanner code={errorCode} raw={errorRaw} />}
        ...
        <div className="flex-1 min-w-0 space-y-5">
          ...
          <section aria-labelledby="step-plan-heading">
            ...
            {loadingTiers ? (
              ...
              <>
                {/* Pricing note — only relevant if Producer-style plan is present */}
                {tiers.some(
                  (t) =>
                    parseFloat(formatUsdMicrosSummary(t.includedUsdMicros).replace("$", "")) >
                    parseFloat(t.monthlyFeeUsd),
                ) && (
                  <p className="mb-3 text-xs text-zinc-600">
                    Included usage on entry plans covers testing; production
                    throughput scales with the full plan.
                  </p>
                )}
                <div
                  role="radiogroup"
                ...
                >
                  {tiers.map((tier) => (
                    <TierCard
                      ...
                      onSelect={() => {
                        setSelectedKeyWithRotate(tier.key);
                        router.replace(
            ...
            {hasPaymentMethod && paymentMethod ? (
              ...
              <div className="rounded-xl border border-white/6 bg-white/2.5 px-4 py-4">
                ...
                <p className="mt-0.5 text-xs text-zinc-600">
                  Adding a card does not charge you. You confirm the charge in
                  the next step.
                </p>
                <button
                  type="button"
          ...
          {/* Confirm — mobile order summary + button */}
