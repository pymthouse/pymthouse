...
/**
 * Opaque `pmth_*` sessions and NaaP composite `app_XXX.pmth_YYY` bearers are
 * tried before the OIDC/JWKS verifier so payment generation accepts the same
 * credentials as `/sign-orchestrator-info`.
 */
function buildFirstMatchEndUserVerifier(
  source: Record<string, string | undefined>,
  original: EnvSource,
  options: RemoteSignerWebhookConfigOptions = {},
): EndUserVerifier {
  const jwtIssuer = trimEnv(source, "OIDC_ISSUER");
  const oidcVerifier = buildEndUserVerifier(source, original);
  ...
  return createFirstMatchEndUserVerifier([
...
export function buildRemoteSignerWebhookConfig(
  envOrOptions?: NodeJS.ProcessEnv | Record<string, string | undefined> | RemoteSignerWebhookConfigOptions,
): RemoteSignerWebhookConfig {
  const options: RemoteSignerWebhookConfigOptions =
    envOrOptions && "env" in envOrOptions
      ? envOrOptions
      : { env: envOrOptions as NodeJS.ProcessEnv | Record<string, string | undefined> | undefined };
  ...
  const config: RemoteSignerWebhookConfig = {
    ...
    endUserAuth: withPhaseTiming(
      withOwnerBillingUsageSubject(
        buildFirstMatchEndUserVerifier(source, original, options),
      ),
    ),
  };
