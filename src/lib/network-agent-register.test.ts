...
function withFlag(value: string | undefined, fn: () => void | Promise<void>) {
  const prior = process.env.NETWORK_AGENT_REGISTER_ENABLED;
  return (async () => {
    if (value === undefined) {
      delete process.env.NETWORK_AGENT_REGISTER_ENABLED;
    } else {
      process.env.NETWORK_AGENT_REGISTER_ENABLED = value;
    }
    try {
      await fn();
    } finally {
      if (prior === undefined) {
        ...
        process.env.NETWORK_AGENT_REGISTER_ENABLED = prior;
      }
    }
  })();
}
...
test("isNetworkAgentRegisterEnabled defaults off", async () => {
  await withFlag(undefined, () => {
    assert.equal(isNetworkAgentRegisterEnabled(), false);
  });
  await withFlag("1", () => {
    assert.equal(isNetworkAgentRegisterEnabled(), true);
  });
});
