...
/**
 * Resolve a `pmth_*` API key to its app/user binding by token alone (no public
 * client scoping). Used by the remote-signer webhook's API-key bearer path,
 * where the caller forwards an opaque credential rather than a JWT.
 */
export async function resolveActiveAppApiKeyByToken(
  bearerToken: string,
): Promise<ResolvedAppApiKey | null> {
  const token = bearerToken.trim();
  if (!token.startsWith("pmth_")) {
