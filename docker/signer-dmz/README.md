...
| `pymthouse/signer-dmz:latest` | `Dockerfile` → `signer-dmz` (default) | `livepeer/go-livepeer:sha-469cf754a259b34927f8dc8d634da3b005493076` | **Production** (Railway / Render) and prod smoke tests |
| `pymthouse-signer:local` | `Dockerfile.signer` | `livepeer/go-livepeer:sha-469cf754a259b34927f8dc8d634da3b005493076` | **Livepeer only** (no Apache; not for public deploy) |
...
2. **JWKS** (CLI auth): the DMZ container must fetch the same keys the app signs with (`JWKS_URI`; default rewrites `localhost` → `host.docker.internal` on Linux via `extra_hosts` in repo `docker-compose.yml`). Remote-only DMZ needs a **reachable** JWKS URL (tunnel, public URL, or VPN), not loopback on the PymtHouse laptop. HTTPS JWKS always verifies TLS certificates — for local/self-signed JWKS use **`http://host.docker.internal/...`** (or `http://localhost/...`) instead of disabling verification.
